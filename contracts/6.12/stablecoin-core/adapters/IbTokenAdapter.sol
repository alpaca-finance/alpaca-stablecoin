...
contract IbTokenAdapter is FarmableTokenAdapter {
    FairlaunchLike  immutable public fairlaunch;
    ShieldLike         immutable public shield;
    ...
    */
    constructor(
        ...
        public
        FarmableTokenAdapter(government_, collateralPoolId_, collateralToken_, rewardToken_)
    {
        // Sanity checks
    ...
    function deposit(address positionAddress, address usr, uint256 val) public override {
        super.deposit(positionAddress, usr, val);
        fairlaunch.deposit(address(this), pid, val);
    }

    function withdraw(address urn, address usr, uint256 val) public override {
        if (live == 1) {
    ...
    function cage() override public {
        ...
        require(
            ...
            fairlaunch.owner() != address(shield) ||
