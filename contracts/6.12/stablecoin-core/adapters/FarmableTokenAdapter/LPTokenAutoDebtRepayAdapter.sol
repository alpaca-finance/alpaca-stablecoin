  ...
  ICagable
{
  ...
  // this will be triggered when beneficialVaultToken equals to stablecoinToken.
  ...
  function _reinvest(
    ...
    // 4. Convert all the remaining rewards to stablecoinToken according to config path.
    router.swapExactTokensForTokens(reward.sub(bounty), 0, getReinvestPath(), address(this), now);

    // 5. Repay all positions' debt of this collateral pool with received stablecoinToken
    stablecoinToken.safeApprove(stablecoinAdapter, actualStablecoinTokenBalance());
    IStablecoinAdapter(stablecoinAdapter).deposit(address(this), actualStablecoinTokenBalance(), abi.encode(0));
    bookKeeper.repayLoan(collateralPoolId, systemDebtEngine, actualStablecoinTokenBalance(), RAY);
  ...
  function getReinvestPath() public view returns (address[] memory) {
    ...
    path[2] = address(stablecoinToken);
    return path;
  }
  ...
  /// thus the actualStablecoinTokenBalance exists to differentiate an actual base token balance balance without taking buy back amount into account
  function actualStablecoinTokenBalance() internal view returns (uint256) {
    return stablecoinToken.myBalance().sub(buybackAmount);
  }
  ...
  function _rewardToBeneficialVault(uint256 _beneficialVaultBounty, uint256 _callerBalance) internal {
    ...
    /// 3. if beneficialvault token not equal to stablecoinToken regardless of a caller balance, can directly transfer to beneficial vault
    ...
    if (beneficialVaultToken != stablecoinToken) {
      buybackAmount = 0;
  ...
  function _deposit(
    ...
    _reinvest(treasuryAccount, treasuryFeeBps, actualStablecoinTokenBalance(), reinvestThreshold);
